...
describe("vertex-rag module", () => {
  ...
  describe("retrieveContext", () => {
    ...
    it("returns mapped chunks when fetch succeeds", async () => {
      ...
      expect(result[0]).toEqual({ text: "Sinhala greeting", source: "gs://bucket/file.txt", score: 0.9 });
      expect(result[1]).toEqual({ text: "Another chunk", source: "gs://bucket/file2.txt", score: 0.75 });
    });
    ...
    it("fills in defaults for missing chunk fields", async () => {
      const responseBody = {
        contexts: {
          contexts: [{ /* no text, no sourceUri, no score */ }],
        },
      };
      vi.stubGlobal("fetch", vi.fn().mockResolvedValue(mockResponse(200, responseBody)));
      const { retrieveContext } = await import("./vertex-rag");
    ...
    it("uses a custom corpusId when provided", async () => {
      ...
      expect(
        calledBody.vertex_rag_store.rag_resources[0].rag_corpus
      ).toContain("custom-corpus-id");
    });
  });
