...
export const QuizHistory = ({ history, stats }: QuizHistoryProps) => {
  const [selectedSession, setSelectedSession] =
    useState<SessionWithQuestions | null>(null);
        ...
        <ScrollArea className="max-h-80 rounded-2xl border border-slate-200 bg-white p-2">
          {history.length === 0 ? (
            <p className="p-2 text-sm text-neutral-500">
              You haven&apos;t taken any AI quizzes yet. Start one to see your
              progress here.
            </p>
          ) : (
            <div className="space-y-2">
        ...
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-hidden">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold">
              {selectedSession
                ? `Review: ${selectedSession.topic}`
                : "Quiz review"}
            </DialogTitle>
            <DialogDescription className="text-xs text-neutral-500">
              Review how you answered each question compared to the correct
              answer.
            </DialogDescription>
          </DialogHeader>
