...
const mockDbQuery = vi.hoisted(() => ({
  challengeOptions: { findFirst: vi.fn() },
}));
...
vi.mock("@/db/drizzle", () => {
  ...
  return {
    default: {
      ...
      query: {
        challengeOptions: {
          findFirst: mockDbQuery.challengeOptions.findFirst,
        },
      },
    },
  };
...
const makeParams = (challengeOptionId: string) => ({
  params: Promise.resolve({ challengeOptionId }),
});

describe("GET /api/challengeOptions/[challengeOptionId]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns 401 when not admin", async () => {
    ...
    const response = await GET(
      new Request("http://localhost/api/challengeOptions/1") as any,
      makeParams("1")
    );
  ...
  it("returns data when admin", async () => {
    ...
    mockDbQuery.challengeOptions.findFirst.mockResolvedValue(mockData);
...
describe("PUT /api/challengeOptions/[challengeOptionId]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns 401 when not admin", async () => {
    ...
    const request = new Request("http://localhost/api/challengeOptions/1", {
      method: "PUT",
...
describe("DELETE /api/challengeOptions/[challengeOptionId]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("deletes and returns data when admin", async () => {
    mockGetIsAdmin.mockResolvedValue(true);
    const mockDeleted = { id: 1, text: "Option 1" };
