...
vi.mock("@/db/drizzle", () => {
  ...
  return {
    default: {
      ...
      delete: mockDbDelete,
      query: {
        courses: { findFirst: mockDbQuery.courses.findFirst },
      },
    },
  };
});
...
describe("GET /api/courses/[courseId]", () => {
  ...
  it("returns 401 when not admin", async () => {
    ...
    const response = await GET(
      new Request("http://localhost/api/courses/1") as any,
      makeParams("1")
    );
  ...
  it("returns data when admin", async () => {
    ...
    mockDbQuery.courses.findFirst.mockResolvedValue(mockData);
...
describe("PUT /api/courses/[courseId]", () => {
  ...
  it("returns 401 when not admin", async () => {
    ...
    const request = new Request("http://localhost/api/courses/1", {
      method: "PUT",
...
describe("DELETE /api/courses/[courseId]", () => {
  ...
  it("deletes and returns data when admin", async () => {
    mockGetIsAdmin.mockResolvedValue(true);
    const mockDeleted = { id: 1, title: "Course 1" };
