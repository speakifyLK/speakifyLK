...
describe("GET /api/units", () => {
  ...
  it("returns empty Content-Range when filter.id returns no results (getMany)", async () => {
    ...
    expect(response.headers.get("Content-Range")).toBe("units */0");
  });
  ...
  it("returns empty Content-Range when range exceeds data (getList)", async () => {
    mockGetIsAdmin.mockResolvedValue(true);
    const allData = [{ id: 1, title: "Unit 1" }];
    ...
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual([]);
    expect(response.headers.get("Content-Range")).toBe("units */1");
  });
  ...
  it("filters by q search term (case-insensitive)", async () => {
    mockGetIsAdmin.mockResolvedValue(true);
    const allData = [
      { id: 1, title: "Basic Vocabulary" },
      { id: 2, title: "Advanced Topics" },
      { id: 3, title: "Basic Phrases" },
    ];
    mockDbQuery.units.findMany.mockResolvedValue(allData);

    const response = await GET(buildRequest({ filter: JSON.stringify({ q: "basic" }) }));
