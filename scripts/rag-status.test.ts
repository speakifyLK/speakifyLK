...
describe("rag-status script", () => {
  ...
  it("reports fully synced when GCS and corpus match", async () => {
    ...
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ name: "corpus", displayName: "Corpus" }),
    });
    // 2. GCS list
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ items: [{ name: "rag-content/lesson.json", md5Hash: "fakehash" }] }),
    });
    // 3. RagFiles
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        ragFiles: [
          { gcsSource: { uris: ["gs://speakifylk-rag-content/rag-content/lesson.json"] } },
        ],
      }),
    });
